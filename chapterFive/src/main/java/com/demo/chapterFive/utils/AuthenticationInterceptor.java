package com.demo.chapterFive.utils;

import java.util.Map;

import com.demo.chapterFive.utils.Struts2PortfolioConstants;
import com.demo.chapterFive.utils.User;
...
public class AuthenticationInterceptor implements Interceptor {
	...
	private void purgeStaleTokens (Map session ){
		...
		if ( !( userToken instanceof User ) ) { 
			session.remove (Struts2PortfolioConstants.USER ) ;
		    System.out.println("Removing stale User token: old token class = " + userToken.getClass());
		}
